...
public class ${className}ServiceBean{
	...
	public int searchCount(Map paramMap ){
		if(logger.isDebugEnabled()){
			logger.debug("${className}ServiceBean.searchCount(Map paramMap ) start-->", paramMap);
		}
		if(ObjectUtils.isNullOrEmpty(paramMap)){

		}

		int result =${classNameLower}Dao.searchcount(paramMap);
		if(logger.isDebugEnabled()){
			logger.debug("${className}ServiceBean.searchCount(Map paramMap ) end-->", result);
		}
		return result;
	}
	 ...
	 * @return List<Name>
	 */
	public int delete(${table.idColumn.simpleJavaType} id){
		if(logger.isDebugEnabled()){
			logger.debug("${className}ServiceBean.delete(int id ) start-->id:{}", id);
		}
		if(ObjectUtils.isNullOrEmpty(id)){

		}

		int result =${classNameLower}Dao.delete(id);
		if(logger.isDebugEnabled()){
			logger.debug("${className}ServiceBean.delete(int id ) end-->result:{}", result);
		}
		return result;
	}
