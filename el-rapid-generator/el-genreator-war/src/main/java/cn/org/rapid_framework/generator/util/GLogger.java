...
public class GLogger {
	...
	public static void println(String projectName,String s) {
		if (logLevel <= INFO) {
			out.println(s);
			try {
				WebsocketHandler.sendMessageToUser(projectName, new TextMessage(new GsonBuilder().disableHtmlEscaping().create().toJson(s)));
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
