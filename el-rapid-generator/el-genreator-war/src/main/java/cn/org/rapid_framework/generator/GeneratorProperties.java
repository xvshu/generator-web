...
public class GeneratorProperties {
	...
	public static void reload() {
		try {
			GLogger.println(projectName,"Start Load GeneratorPropeties from classpath:"+Arrays.toString(PROPERTIES_FILE_NAMES));
			Properties p = new Properties();
			String[] loadedFiles = PropertiesHelper.loadAllPropertiesFromClassLoader(p,PROPERTIES_FILE_NAMES);
			GLogger.println(projectName,"GeneratorPropeties Load Success,files:"+Arrays.toString(loadedFiles));
			setProperties(p);
		}catch(IOException e) {
...
//            }
	...
	public static void setProperty(String key,String value) {
	    ...
	    GLogger.println(projectName,"[setProperty()] "+key+"="+value);
	...
	public static void setProperties(Properties inputProps) {
        ...
        for(Iterator it = props.entrySet().iterator();it.hasNext();) {
            ...
            GLogger.println(projectName,"[Property] "+entry.getKey()+"="+entry.getValue());
        }
        GLogger.println(projectName,"");
        
        GLogger.println(projectName,"[Auto Replace] [.] => [/] on generator.properties, key=source_key+'_dir', For example: pkg=com.company ==> pkg_dir=com/company  \n");
