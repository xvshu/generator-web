...
public class CasFilter extends AuthenticatingFilter {
	...
	protected boolean onAccessDenied(ServletRequest request,
		...
		if(StringUtils.isAllLowerCase(SAMLRequest)){
			logoutMessage = uncompressLogoutMessage(CommonUtils.safeGetParameter(httpRequest,"SAMLRequest"));
		}else{
			logoutMessage = CommonUtils.safeGetParameter(httpRequest, "logoutRequest");
		}
		if(StringUtils.isNotEmpty(logoutMessage)){
			final String token=XmlUtils.getTextForElement(logoutMessage, "SessionIndex");
			Serializable sessionId =(Serializable)cacheManager.getCache(TICKET_LOGOUT).get(token);
			if(null!=sessionId){
				SimpleSession ss =new SimpleSession();
				ss.setId(sessionId);
				redisSessionDAO.delete(ss);
				if (logger.isInfoEnabled())
					logger.info("remove session Attribute：{}",DelegatingSubject.class.getName() + ".RUN_AS_PRINCIPALS_SESSION_KEY");
			}
		}
	...
	protected boolean onLoginSuccess(AuthenticationToken token,
		...
		if (logger.isInfoEnabled())
			logger.info("storage session ticket：{}",ticket);
		cacheManager.getCache(TICKET_LOGOUT).put(ticket,
				subject.getSession().getId());
		cacheManager.getCache(TICKET_SESSION).put(subject.getSession().getId(),ticket);
