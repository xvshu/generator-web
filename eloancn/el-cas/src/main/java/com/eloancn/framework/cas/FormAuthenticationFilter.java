...
public class FormAuthenticationFilter extends AuthenticatingFilter {
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
    	...
    	//String url_m=req.getScheme()+"://"+req.getServerName()+"/cas";
    	//SessionUtils.getSession().setAttribute("cas_url", url_m);
    
    	//this.setLoginUrl(this.getLoginUrl().substring(0,this.getLoginUrl().indexOf("=")+1)+url_m);
        if (isLoginRequest(request, response)) {
            ...
            this.setLoginUrl(this.getLoginUrl().replaceAll("&decorator=remote", ""));
            int index=this.getLoginUrl().indexOf("&v=");
            String _url;
            if(index>0){
            	_url=this.getLoginUrl().substring(0, index);
            }else{
            	_url=this.getLoginUrl();
            }
            if(popUrl!=null&&popUrl.contains(url)){
            	this.setLoginUrl(_url+"&decorator=remote&v="+System.currentTimeMillis());
            }else{
            	this.setLoginUrl(_url+"&v="+System.currentTimeMillis());
            }
        	String cas=req.getHeader("cas");
        	if(StringUtils.isNotEmpty(cas)){
        		HttpServletResponse res=(HttpServletResponse)response;
        		res.setHeader("cas", this.getLoginUrl());
            	return false;
        	}	
